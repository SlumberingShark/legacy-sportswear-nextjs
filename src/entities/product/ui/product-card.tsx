...
export function ProductCard({ product, onAddToCart }: ProductCardProps) {
	const { name, price, fabric, colors, image, inStock = true } = product;
	const [isFavorite, setIsFavorite] = useState(false);

	return (
		<div className="group relative flex flex-col bg-white overflow-hidden transition-all duration-300">
			{/* Контейнер изображения (Соотношение 3/4, скругление xl как в Figma) */}
			<div className="relative aspect-[3/4] bg-zinc-100 overflow-hidden rounded-xl mb-4">
				...
				{/* Кнопка лайка (Клик изолирован через e.preventDefault) */}
				<button
					onClick={(e) => {
						e.preventDefault();
						setIsFavorite(!isFavorite);
					}}
					className="absolute top-3 right-3 bg-white text-black rounded-full p-2.5 shadow-sm md:opacity-0 md:group-hover:opacity-100 transition-all duration-200 hover:scale-105 active:scale-95 z-10"
				>
					<Heart className={`w-4 h-4 transition-colors ${isFavorite ? "fill-black text-black" : "text-black"}`} strokeWidth={1.5} />
				</button>
			...
			{/* Информационный блок без лишних боковых паддингов (выровнен по картинке) */}
			<div className="flex flex-col flex-grow space-y-1 px-1">
				...
				{/* Цена */}
				<p className="text-sm font-normal text-black pt-1">{typeof price === "number" ? (price / 100).toLocaleString("ru-RU") : price} ₽</p>
